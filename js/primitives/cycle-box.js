// 牛市/熊市半透明矩形 + 角标签。projected（预测）段用虚线边框 + 减半填充。
...
export class CycleBox extends Primitive {
  ...
  constructor({ from, to, priceLow, priceHigh, fill, label, labelPos = 'top', dashed = false }) {
    super('bottom');
    this._from = from;
    this._to = to;
  ...
  _draw(ctx, media) {
    ...
    ctx.lineWidth = 1;
    ctx.setLineDash(this._dashed ? [6, 5] : []);
    ctx.strokeRect(cx1, cy1, cx2 - cx1, cy2 - cy1);
    ctx.setLineDash([]);

    if (!this._label) return;
    // 框缘滚出屏幕时标签贴住视口边缘，保持可见（上缘避开左上角的标题浮层）
    const lx = Math.max(cx1, 0) + 8;
    if (cx2 - lx < 48) return; // 框太窄/基本滚出视口时不画标签
    ctx.font = `bold 15px ${FONT}`;
    ctx.fillStyle = COLORS.boxLabel;
    ctx.textAlign = 'left';
    if (this._labelPos === 'top') {
      ctx.textBaseline = 'top';
      ctx.fillText(this._label, lx, cy1 < 0 ? 64 : cy1 + 8);
    } else {
