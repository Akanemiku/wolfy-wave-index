# 杀破狼 WolfyXBT · 比特币四年周期理论

把手绘的比特币四年周期图做成可实时浏览的网站：完整 BTC 日线 K 线（2013 年至今，
对数坐标，可缩放/平移），图上刻有固定标注——牛市/熊市区间、减半日竖线、周期时长
箭头、牛顶气泡，以及按 364 天规律推演的预测熊市区间（虚线）。标注只能看，不能改。
...
- 箭头文字：`ARROW_LABEL_MODE = 'fixed'`（原图数字）或 `'computed'`（实际天数）
- 配色：`COLORS`（想改红涨绿跌就交换 `up`/`down`）
